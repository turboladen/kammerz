<script lang="ts">
	...
	const selectedCamera = $derived(cameraId ? cameras.find((c) => c.id === Number(cameraId)) : null);

	// Hint for medium format variable-back cameras and large format sheet film
	const frameCountHint = $derived.by(() => {
		const matchingFormat = selectedCamera ? cameraFormatToStockFormat[selectedCamera.format] : null;
	...
	const filmStockOptions = $derived.by(() => {
		...
		const options: { value: string; label: string; disabled?: boolean }[] = [{ value: '', label: 'Select film stock' }];
