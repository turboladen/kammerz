...
- `just deploy <user>@<host>` — Release = deploy: cross-compiles for the aarch64 DietPi server (`just build-linux`; messense gcc toolchain + rustup target, linker in `.cargo/config.toml`), streams the binary and systemd unit over ssh, restarts the service, and verifies via `GET /api/health` reporting the new version. There are no GitHub releases.
